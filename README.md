...
rename and set in [rememberthemilk.toml.example](src/rememberthemilk.toml.example)
...
run [uv run src/auth.py](src/auth.py) once and add the resulting `token` to `rememberthemilk.toml`
